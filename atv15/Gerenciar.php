<h3 class='p-2 m-2 bg-dark text-warning text-center'>Gerenciar Funcionários</h3>
<p class='m-2'>
...
if ( isset($_GET['acao'])) {
	...
	elseif ( $acao == "L" )	{	# LISTAR RESULTADO DA BUSCA
	...
	elseif ( $acao == "R" )	{	# GERAR RELATORIOS
		...
		if ( isset($_GET['rel'])) {		# EXIBE O RELATORIO
			...
			elseif ( $_GET['rel'] == 'REL_Estagio' ) {
                ...
                $titulo = "Relatório total por setor - '$estagio'";
                if($estagio == TRUE)
                    $titulo = "Relatório total de estagiarios";
                else
                    $titulo = "Relatorio total de funcionarios efetivos";
			}
			//funSelect($tabela, $campos, $argumentos) 	
            $tabela = funSelect("tb_func", "*", $argumentos); 		
